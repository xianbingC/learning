...
- 按值的默认捕获模式[=]，会对作用域内的局部变量按值进行捕获；按值捕获a和b（[a,b]）、按引用捕获作用域内的全部变量（[&]）、按引用捕获a和b（[&a,&b]）；按引用捕获a，按值捕获其他变量（[=,&a]）
- 特别注意，默认的按值或者按引用的捕获模式均会捕获到this指针，这就导致可能会出现指针空悬的可能，所以应该使用指定捕获需要捕获的变量
- 无法捕获静态变量（全局、局部、类成员），但是可以使用
- 无法捕获类的非静态成员变量，如需要使用可通过捕获this指针（不建议）或创建一个成员变量的副本进行捕获

### 内存管理
- 对于32位系统而言，内存大小为4G，程序在内存中的布局如下：
![C++内存布局](内存布局.jpg)
    - 代码段：可执行代码、字符串常量
    - 数据段：包括data段和bss段，前者放置已初始化的全局变量或静态变量，后者存放未初始化的全局变量和静态变量

- 动态内存在堆区申请，通过new、new[]或malloc申请、由delete、delete[]或free释放
- malloc实现原理
    1. malloc(0)对于32为系统可用大小至少为12字节，64位系统至少为24字节。
    2. 对齐方式，32位系统按8字节对齐，64位系统按16字节对齐。
    3. malloc的原理：先申请一大块内存，然后将内存分成大小不等的数据块，通过隐式链表连接起来。当调用malloc申请内存时，通过遍历链表的方式寻找一块大小合适的且未使用的内存块，然后返回该内存块的首地址（控制块之后的地址即为首地址）。如果没找到，那么就会调用系统调用brk（小于128K）或mmap（大于128K）来分配一块指定大小的内存块。成功则返回内存块首地址，否则返回NULL。其中brk使得堆向高地址增长，而mmap则是在栈和堆之间分配一块内存。
- free实现原理
    1. free的原理：通过内存首地址找到控制块的首地址，然后置控制块的可用标志位为1即可。也就是说在释放之后该段内存的数据还在，仍然可以读出来，但不安全，因为该段内存随时都有可能被分配走。
- 内存碎片：通过brk分配的内存必须待高地址释放后，低地址的内存块才能释放，因此当频繁的分配和释放内存时，就会造成内存碎片化，即不连续。当连续空闲块的内存超过128K时就会执行内存紧缩，释放堆内存。
- 局部变量的内存由内核管理，在栈区进行
